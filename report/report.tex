...
$R$ is returned if both players cooperate, $S$ is set if the player that is watching the matrix cooperates and the other defects, $T$ is for the opposite case and finally $P$ is the payoff for both players if they both defect.
...
\FR{cite also \cite[intro, colman par.]{yao1994experimental}}
...
Axelrod also reported \textit{TfT}, a strategy submitted to him by Rapoport, to be the winner over all the strategies analyzed by him in his extensive tests based on tournaments and evolution on the IPD, thoroughly described in~\cite{axelrod1981evolution,axelrod1984evolution}.
...
In short, it is a variation of the previous case, in which multiple players play in a RR way. The variation consists in the fact that a single player will win the tournament if at the end of it he has the highest cumulative payoff. The culumative points after each match are shown in \autoref{}. \FR{fixme}
...
\FR{\cite[2.6]{mathieu2017} contains material that can be cited here. A paragraph may be dedicated in the end to \cite[intro]{IPDnoise}}
...
\FR{Mention the results shall be comparable with the paper \cite{mathieu2017}.}
