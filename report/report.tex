...
It can be easily seen how the \textit{GrT} and \textit{TfT} strategies very quickly outpaces the others: in just two iterations they represent almost half the population with a predominance of \textit{TfT} players. In the fifth we can see that \textit{GrT} takes the lead. These results add something to the previous results obtained by the simulation of the iterated Prisoner's Dilemma: the \textit{TfT} overwhelm its brother \textit{Tf2T}. Looking at the scores instead we can see how \textit{GrT} and \textit{TfT} are pretty similar since they do not trigger each other. Finally, as before, results depend on the initial population as we can see for example fixing the seed to \textbf{finish} and in \textbf{figure}.
...
%\begin{figure}[!ht]
%    \centering
%    \includegraphics[width=1\columnwidth]{../img/ripdmp-const/ripdmp-evolution-const-pop-50-bis}
 %   \caption{Evolution of rIPDMP, constant population of 50, random seed}
 %   \label{fig:constRrandom}
%\end{figure}
