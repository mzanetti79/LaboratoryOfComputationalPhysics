...
Winning a game in this setup simply means achieving eventually a better payoff with respect to the opponents and this could be achieved even without playing such strategies.
...
As a matter of fact, in all scenarios, \textit{Bad guy} reaches at least the same reward of the opponent, but more often he gets an higher one as in Figures~[\ref{fig:badvstft},\ref{fig:badvsnice},\ref{fig:badvsmainlybad}]. 
...
On the contrary, taking the averages of these two metrics with respect to all the subsequent matches, it can be seen how the only strategies that achieve high performance on both are the \textit{Tf(2)T} and \textit{GrT}, meaning that even if they do not win every time, they achieve pretty high payoffs. %\textbf{It's like loosing battles but winning wars}
...
Each player has a gene $c$, representing his attitude to cooperate. This value is initially generated as $k/100$ where $k$ is the probability of cooperating for random, \textit{Bad} and \textit{Nice} strategies. For \textit{GrT}, \textit{TfT} and \textit{Tf2T} $c$ is set to $0.5$ since these strategies does not have inside a specific attitude for cooperating.
...
A single round of the game goes as follow: a rIPDMP's round is played, then new players are generated following alternative $1$, then for the "old" players a new $c$ is generated and their strategies changes accordingly. This will be repeated up until convergence or when a max number of iterations is reached.


Two alternatives are proposed to change $c$ after each round.
\begin{enumerate}
    \item For each player, a new random $c$ is generated
...
At this point if the absolute value of the difference between the old $c$ and the new $c$ is greater than a threshold (set to $0.1$) the strategy will change. 
...
These random strategies are bounded based on the previous and the new $c$
\begin{itemize}
\item New $c$ is $<0.5$ we stay on the "bad" side
	\begin{enumerate}
	\item $[(1-c)\times 100, \min(id,50)]$ if going more cooperative
	\item $[\min(id,50), (1-c)\times 100]$ if going less cooperative
	\end{enumerate}
\item New $c$ is $>=0.5$ we stay on the "good" side
	\begin{enumerate}
	\item $[\max(id,50), (1-c)\times 100]$ if going less cooperative
	\item $[(1-c)\times 100, \max(id,50)]$ if going more cooperative
	\end{enumerate}
\end{itemize}
...
\begin{table}[ht]
    ...
    \begin{tabular}{l|cccccc} \toprule
        ...
        MainlyBad (k=97)     &   1 &   1 &    1 &    1 &    1 &    1 \\
        MainlyBad (k=98)     &   1 &   1 &    1 &    1 &    1 &    1 \\
        MainlyBad (k=99)     &   1 &   2 &    2 &    2 &    2 &    2 \\% \midrule
        ...
        MainlyBad (k=79)     &     &     &    1 &    1 &    1 &    2 \\% \midrule
        ...
        % alternative compact version for strategies entered last round
        % saves space to allow back the use of midrules
        % MainlyNice (k=12,23,30,35,39)         & & & & & & 1 \\
        % MainlyNice (k=41,55)                  & & & & & & 2 \\
        % MainlyBad (k=60,66,69,73,75,76,83,90) & & & & & & 1 \\
        % MainlyBad (k=71)                      & & & & & & 2 \\ \midrule
    \end{tabular}
\end{table}
