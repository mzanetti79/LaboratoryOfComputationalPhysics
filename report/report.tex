...
\begin{figure}[!ht]
    ...
    \label{fig:constLI}
\end{figure}
\textbf{Discussion citing figures as for subsec below}

\textbf{ADD TABLE}

\subsection{Increasing Population}
In this case the number of players (population) is increased at each iteration. We implemented three different way of modifying the population between rounds: the method may be statically chosen by setting the \texttt{ALTERNATIVE} variable.

After each round a player has a certain probability based on his ranking to have a child of the same type:
\begin{enumerate}
...
\end{enumerate}
We used by default the first of the three proposed methods.
